...
├── tests/                  ← automated test suite (1033+ tests)
...
- `row.py`, `column.py`, `grid.py`, `anchor.py` — stateless helpers
- `flex.py` — `FlexRow`, `FlexColumn` stateful responsive helpers
- `anchor_layout.py` — `AnchorLayout` stateful anchor rules
...
- `audio_manager.py`: `AudioManager` — music, SFX, volume
- `positional.py`: `PositionalAudio`, `PositionalSource` — distance falloff, stereo panning
...
- `state_machine.py`: `AnimationStateMachine` — state/transition/condition driven animation
...
- `pathfinder.py`: `ObstacleGrid`, `Pathfinder` — A* with 4/8-dir, corner prevention, `from_tilemap()`

### `lighting/`
2D lighting overlay.
- `lighting.py`: `LightingSystem`, `Light` — dark overlay, radial gradients, flicker, camera-aware

### `camera/`
2D world/screen coordinate conversion.
- `camera.py`: `Camera` — position, zoom, smooth follow, screen shake, bounds clamping

### `tilemap/`
2D tile map rendering and collision.
- `tileset.py`: `Tileset` — spritesheet slicing
- `layer.py`: `TileLayer` — named 2D grid of tile indices
- `tilemap.py`: `Tilemap` — multi-layer rendering, camera culling, collision

### `dialogue/`
Script-driven dialogue system.
- `script.py`: `DialogueScript`, `DialogueNode`, `Choice` — validated script format
- `runner.py`: `DialogueRunner` — pure state machine
- `box.py`: `DialogueBox` — typewriter widget with choices

### `atlas/`
Sprite atlas packing.
- `atlas.py`: `AtlasPacker`, `SpriteAtlas` — pack and blit named sprites

### `locale/`
String localisation.
- `locale_store.py`: `LocaleStore` — key lookup, pluralisation, hot-swap
...
- Complete UI toolkit: 15 widget types with focus traversal and overlay render pass
- Animation: Tween (30 easings), SpriteAnimation, AnimationPlayer
- Assets, Audio, Persistence (with migrations), Particles, EventBus
- Debug tools: overlay, console, inspector, debug log, crash logging
- Camera: world/screen conversion, smooth follow, screen shake, bounds
- Tilemap: multi-layer rendering, camera culling, tile collision
- Dialogue: script-driven, typewriter, branching, callbacks
- Responsive layout: FlexRow, FlexColumn, AnchorLayout
- Sprite atlas packing and localisation
- Pathfinding: A* on ObstacleGrid with Tilemap integration
- Animation state machine: condition-driven transitions between animation states
- 2D positional audio: distance falloff and stereo panning
- 2D lighting: dark overlay with radial gradient light sources
